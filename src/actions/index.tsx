...
export const fetchQuery = (
  ...
  nextPageToken?: string,
): commonTypes.DispatchAction => async (dispatch, getState) => {
    ...
    const params: types.IFetchParams = {
      key: process.env.REACT_APP_YT_KEY as string,
      maxResults: "12",
      part: "snippet",
      q: query,
      type: "video",
...
export const selectVideoSetUrl = (
  video: commonTypes.ISearchResource,
  index: number): commonTypes.DispatchAction => async (dispatch, getState) => {
...
export const selectPrevVideoSetUrl = (
  searchResults: commonTypes.ISearchResource[],
  index?: number): commonTypes.DispatchAction => async (dispatch) => {
    fromNullable(index)
      .map((i) => {
        let newVideoIndex = i;
        newVideoIndex--;
...
export const selectNextVideoSetUrl = (
  searchResults: commonTypes.ISearchResource[],
  index?: number): commonTypes.DispatchAction => async (dispatch, getState) => {
...
export const more = (): commonTypes.DispatchAction => async (dispatch, getState) => {
    const stateBeforeFetch = getState();
    return dispatch(fetchQuery(stateBeforeFetch.query,
      stateBeforeFetch.searchResults.nextPageToken,
    ));
...
export const unselectVideoSetUrl = (query: string): commonTypes.DispatchAction => async (dispatch, getState) => {
...
export const resetSetUrl = (): commonTypes.DispatchAction => async (dispatch, getState) => {
    dispatch(clearQuery());
    dispatch(resetSearchResults());
